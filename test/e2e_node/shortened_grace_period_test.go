...
var _ = framework.KubeDescribe("Shortened Grace Period", func() {
	...
	ginkgo.Context("When repeatedly deleting pods", func() {
		ginkgo.It("should be deleted immediately", func() {
			...
			var gracePeriod int64 = 20
			f.PodClient().CreateSync(getGracePeriodTestPod(podName, nodeName, gracePeriod))
			err := f.PodClient().Delete(context.TODO(), podName, metav1.DeleteOptions{
			...
			f.PodClient().DeleteSync(podName, metav1.DeleteOptions{
				GracePeriodSeconds: &gracePeriodZero,
			}, 1*time.Second)
			framework.ExpectEqual(time.Since(start) < 3*time.Second, true, "cannot update GracePeriodSeconds")
		})
	})
})
